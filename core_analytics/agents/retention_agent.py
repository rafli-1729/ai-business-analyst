...
def retention_agent_node(state: AgentState):
    """LangGraph node function for Retention Agent"""
    ...
    try:
        ...
        system_prompt = "You are a customer retention expert. Use the execute_sql_tool to query the database."
    
    response = agent.invoke({
        "messages": [
            SystemMessage(content=system_prompt),
            HumanMessage(content=f"Please analyze this from a retention and churn perspective: {state['query']}")
        ]
    })
    
    artifacts = parse_agent_response(response["messages"][-1].content, "Retention Agent")
    return {"artifacts": artifacts, "active_agents": ["Retention Agent"]}
...
class RetentionAgent(BaseAgent):
    ...
    async def process(self, query: str) -> List[AgentArtifact]:
        ...
        return retention_agent_node(state)["artifacts"]
