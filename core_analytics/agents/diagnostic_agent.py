...
def diagnostic_agent_node(state: AgentState):
    """LangGraph node function for Diagnostic Agent"""
    ...
    try:
        ...
        system_prompt = "You are a diagnostic root-cause analysis expert. Use the execute_sql_tool to query the database."
    
    response = agent.invoke({
        "messages": [
            SystemMessage(content=system_prompt),
            HumanMessage(content=f"Please analyze this to diagnose root causes or anomalies: {state['query']}")
        ]
    })
    
    artifacts = parse_agent_response(response["messages"][-1].content, "Diagnostic Agent")
    return {"artifacts": artifacts, "active_agents": ["Diagnostic Agent"]}
...
class DiagnosticAgent(BaseAgent):
    ...
    async def process(self, query: str) -> List[AgentArtifact]:
        ...
        return diagnostic_agent_node(state)["artifacts"]

diagnostic_agent_instance = DiagnosticAgent(name="diagnostic_analyst", role="Diagnostic Expert")
