...
def trend_agent_node(state: AgentState):
    """LangGraph node function for Trend Agent"""
    ...
    try:
        ...
        system_prompt = "You are a trend analysis expert. Use the execute_sql_tool to query the database."
    
    response = agent.invoke({
        "messages": [
            SystemMessage(content=system_prompt),
            HumanMessage(content=f"Please analyze this from a time-series and trend perspective: {state['query']}")
        ]
    })
    
    artifacts = parse_agent_response(response["messages"][-1].content, "Trend Agent")
    return {"artifacts": artifacts, "active_agents": ["Trend Agent"]}
...
class TrendAgent(BaseAgent):
    ...
    async def process(self, query: str) -> List[AgentArtifact]:
        ...
        return trend_agent_node(state)["artifacts"]
