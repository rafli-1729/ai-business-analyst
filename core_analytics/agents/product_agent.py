...
def product_agent_node(state: AgentState):
    """LangGraph node function for Product Agent"""
    ...
    try:
        ...
        system_prompt = "You are a product domain expert. Use the execute_sql_tool to query the database."
    
    response = agent.invoke({
        "messages": [
            SystemMessage(content=system_prompt),
            HumanMessage(content=f"Please analyze this from a product catalog and category perspective: {state['query']}")
        ]
    })
    
    artifacts = parse_agent_response(response["messages"][-1].content, "Product Agent")
    return {"artifacts": artifacts, "active_agents": ["Product Agent"]}
...
class ProductAgent(BaseAgent):
    ...
    async def process(self, query: str) -> List[AgentArtifact]:
        ...
        return product_agent_node(state)["artifacts"]
